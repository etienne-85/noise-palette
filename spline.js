import { ParameterInput } from "./params";
import * as Utils from "./utils";

class Spline {
  constructor(controls) {
    this.controls = Utils.copy_object_array(controls);
  ...
  eval(t) {
    ...
    for (let i = 0; i < this.controls.length; i++) {
      ...
      if (
        i < this.controls.length - 1 &&
        this.controls[i].x < x &&
        this.controls[i + 1].x > x
      ) {
...
class SplineInput extends ParameterInput {
  ...
  get_click_target(event, collisions = true) {
    ...
    let target = {
      x:
        ((event.clientX - bounds.left - this.padding / width_scale) /
          (this.width - 2 * this.padding)) *
        width_scale,
      y:
        1 -
        ((event.clientY - bounds.top - this.padding / height_scale) /
    ...
    if (collisions) {
      for (let i = 0; i < this.controls.length; i++) {
        if (Utils.collides(this.controls[i], target, hitbox)) {
          target.control = i;
        }
        if (Utils.collides(this.controls[i].hl, target, hitbox)) {
          target.handle = { control: i, index: HANDLE_LEFT };
        }
        if (Utils.collides(this.controls[i].hr, target, hitbox)) {
  ...
  inflate(wrapper) {
    ...
    this.controls = Utils.copy_object_array(this.initial_value);
    ...
    wrapper.addEventListener("mousedown", (e) => {
      if (e.button == 0) self.on_mousedown(e);
    });
    window.addEventListener("mousemove", (e) => {
      self.on_mousemove(e);
    });
    window.addEventListener("mouseup", (e) => {
      if (e.button == 0) self.on_mouseup(e);
    });
    wrapper.addEventListener("contextmenu", (e) => {
      self.on_contextmenu(e);
    });
  }
  ...
  on_handle_drag(target) {
    if (this.moving_handle.index == HANDLE_LEFT) {
      this.controls[this.moving_handle.control].hl = {
        x: target.x,
        y: target.y,
      };
    } else {
      this.controls[this.moving_handle.control].hr = {
        x: target.x,
        y: target.y,
      };
    }
  ...
  draw_control_handle(x0, y0, handle) {
    ...
    let y1 =
      this.height -
      (handle.y * this.inner_height + this.padding + this.dot_size / 2);
  ...
  draw_control(control) {
    ...
    let y0 =
      this.height -
      (control.y * this.inner_height + this.padding + this.dot_size / 2);
    if (control.hl != null) {
  ...
  draw() {
    ...
    for (let i = 1; i < this.inner_width; i++) {
      let y =
        spline.eval(i / (this.inner_width - 1)) * this.inner_height +
        this.padding;
  ...
  write(value) {
    this.controls = Utils.copy_object_array(value);
    this.draw();
  }
