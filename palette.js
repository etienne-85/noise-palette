...
class Palette {
  constructor(stops) {
    this.stops = Utils.copy_object_array(stops);
  ...
  eval(t) {
    ...
    if (this.stops[this.stops.length - 1].t < x)
    ...
    for (let i = 0; i < this.stops.length; i++) {
      ...
      if (
        ...
        return Utils.lerpN(z, this.stops[i].color, this.stops[i + 1].color);
      }
    }
  }
}
...
class PaletteInput extends ParameterInput {
  ...
  inflate(wrapper) {
    ...
    this.stops = Utils.copy_object_array(this.initial_value);
  ...
  write(value) {
    this.stops = Utils.copy_object_array(value);
    this.draw();
  }
